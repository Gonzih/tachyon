...
- **Fields consumed:**
  - `five_hour {utilization%, resets_at ISO8601}` → **ring**. If absent/null → fall back to `limits[]` entry `kind=="session"`; if that's also absent → snapshot unavailable. Never substitute weekly into the ring.
