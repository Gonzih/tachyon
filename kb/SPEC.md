...
- **Pace pulse:** a pace-hot ring window additionally *breathes* — the pill arc and the provider's shim segment oscillate opacity (0.9s ease, autoreversing; arc 1→0.4, shim 1→0.35). An exhausted window (100%) never pulses — the pulse means "act now", and at the wall there is no decision left. Suppressed under Reduce Motion. Shim pulse is a repeating `CABasicAnimation` on a per-segment layer — render-server work, zero app CPU.
